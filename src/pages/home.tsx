...
export default function Home() {
  ...
  // Load previously generated videos (persisted in IndexedDB) on mount.
  useEffect(() => {
    getAllVideos().then((list) => setResults(list.map(storedToResult))).catch(() => {});
    refreshUsage();
  }, []);
  ...
  async function generateOne(index: number, total: number, flags: RunFlags): Promise<VideoResult> {
    ...
    if (isFast) {
      const std = fastStdSize(aspectId, qualityId);
      if (!std) throw new Error("Fast mode doesn't support Square (Pexels has no native square). Use Standard mode or another aspect ratio.");
    ...
    const { blob, duration: outDur, usedClips } = await generateVideo({
      ...
      targetDuration: duration, fast: isFast, music, onProgress,
    });
    ...
    const result: VideoResult = {
      ...
      duration: outDur,
      themeLabel: label,
      aspectLabel: `${aspect.label} · ${fps}fps${isFast ? " · fast" : ""}`,
      musicLabel,
      createdAt: new Date().toISOString(),
      elapsedMs: performance.now() - started,
    };

    // Persist so it survives navigation / reloads.
    await saveVideo({
      id: result.id, blob, duration: outDur, themeLabel: result.themeLabel,
      aspectLabel: result.aspectLabel, musicLabel: result.musicLabel,
      createdAt: result.createdAt, elapsedMs: result.elapsedMs,
    }).catch(() => {});
  ...
  async function handleGenerate() {
    ...
    if (made === 0) {
      ...
      const timeStr = total > 1 ? `${formatElapsed(totalMs)} total (~${formatElapsed(totalMs / made)}/video)` : `in ${formatElapsed(totalMs)}`;
      const desc = failures.length ? `${failures.length} failed; the rest are below. Took ${formatElapsed(totalMs)}.` : `Done ${timeStr}.`;
            ...
            <Card>
              <CardHeader className="pb-4">
                ...
                <p className="text-muted-foreground text-sm">
                  Clips are fetched from Pexels and assembled locally. Batch mode makes several unique videos in a row.
                </p>
              </CardHeader>
              <CardContent className="space-y-8">
                <ThemeSelector themeId={themeId} customQuery={customQuery}
                  onSelectTheme={(id) => { setThemeId(id); setCustomQuery(""); }} onCustomQuery={setCustomQuery} />
                <Separator />
                ...
                {isFast && (
                  <p className="text-xs text-amber-600 dark:text-amber-500 flex items-start gap-1.5 -mt-4">
                    <AlertTriangle className="h-3.5 w-3.5 mt-0.5 shrink-0" />
                    Fast mode joins same-size clips without re-encoding — much faster, but clip transitions may not be perfectly smooth and the length is approximate. Great when the result will be re-encoded later. (Square isn't supported.)
                  </p>
                )}
                <Separator />
                <DurationSlider value={duration} onChange={setDuration} min={10} max={600} step={5} />
                <Separator />
                <MusicSelector source={musicSource} genGenre={genGenre} jamGenre={jamGenre} jamendoReady={jamReady}
                  file={musicFile} volume={musicVolume} onSource={setMusicSource} onGenGenre={setGenGenre}
                  onJamGenre={setJamGenre} onFile={setMusicFile} onVolume={setMusicVolume} />

                {isHeavy && (
                  <p className="text-xs text-amber-600 dark:text-amber-500 flex items-start gap-1.5">
                    <AlertTriangle className="h-3.5 w-3.5 mt-0.5 shrink-0" />
                    Big batches, long durations, 60fps or 1080p take a while to encode in the browser. Try <strong>Fast mode</strong> for a big speed-up. You can switch tabs — you'll get a notification when done. Don't close the tab.
                  </p>
                )}
